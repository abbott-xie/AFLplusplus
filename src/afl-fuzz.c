...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
    ...
    switch (opt) {
      ...
      case 'H':
        afl->use_set_cover_scheduling = 1;
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if(likely(afl->use_set_cover_scheduling && afl->queued_items && afl->queue_cur)){

      // fprintf(afl->fsrv.set_cover_analysis, "cull_queue_new \n");
    ...
    do {
      ...
      if (likely(!afl->old_seed_selection)) {
        ...
        if(likely(afl->use_set_cover_scheduling)){

          afl->current_entry = afl->set_favored_id;

          if (!afl->queue_buf){
            printf("afl->queue_buf is null"); 
          }
          afl->queue_cur = afl->queue_buf[afl->current_entry];
      ...
      // u64 fuzz_one_time = get_cur_time();
      ...
      if (fuzz_one_curr_time - afl->start_time >= 10000 * time_counter) {
        
        fprintf(afl->fsrv.plot_file, "fuzz_one_time : %llu \n", (get_cur_time() - afl->start_time) / 60000);
...
stop_fuzzing:
  ...
  if (afl->not_on_tty) {
    ...
    write_seeds_exec_time_distribution(afl);
    ...
    ACTF(
        ...
        "%llu crashes saved, %llu timeouts saved, total runtime %s, set_cover_call_nums : %u, use_previou_call_nums: %u, cull_queue_new_call_nums:%u,detect_frontier_changes_nums: %u, use_previou_call_and_random:%u,global_covered_frontier_nodes_count:%u", 
        afl->queued_discovered,
        ((double)t_bytes * 100) / afl->fsrv.real_map_size, afl->saved_crashes,
        afl->saved_hangs, time_tmp, afl->set_cover_call_nums,afl->use_previou_call_nums,afl->cull_queue_new_call_nums, afl->detect_frontier_changes_nums, afl->use_previou_call_and_random, afl->global_covered_frontier_nodes_count);
  ...
  fclose(afl->fsrv.set_cover_analysis);
  #ifdef INTROSPECTION
